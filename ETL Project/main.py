...
try:
    ...
    if connection.is_connected():
        ...
        cursor = connection.cursor() #the SQL query for part three of this assignment - gets number of titles associated with each of the countries and returns their average rating
        query = """
                SELECT 
                    C.CountryName, 
                    COUNT(T.TitleID) AS NumberOfTitles, 
                    AVG(R.RatingValue) AS AverageRating
                FROM Titles T
                JOIN Countries C ON T.CountryID = C.CountryID
                JOIN Ratings R ON T.TitleID = R.TitleID
                GROUP BY C.CountryName
                ORDER BY NumberOfTitles DESC, AverageRating DESC;
                """
        cursor.execute(query)
        results = cursor.fetchall()

        # Print the results
        print("CountryName | NumberOfTitles | AverageRating")
        for (countryName, numberOfTitles, averageRating) in results:
            print(f"{countryName} | {numberOfTitles} | {averageRating:.2f}")
        cursor.close()
